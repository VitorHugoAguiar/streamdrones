...
export const detect = (src, canvas, socket) => {
  console.log("canvas.width x canvas.height: " + canvas.width + "x" + canvas.height);
  console.log("src.videoWidth x src.videoHeight: " + src.videoWidth + "x" + src.videoHeight);
  canvas.width = src.width;
  canvas.height = src.height;

  const ctx = canvas.getContext("2d");
  const [modelWidth, modelHeight] = window.modelInputShape.slice(2, 4); // get model input width and height
  console.log("modelWidth x modelHeight: " + modelWidth + "x" + modelHeight);
